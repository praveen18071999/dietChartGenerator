...
const processDietHistoryData = (apiData: Order[]) => {
  return apiData.map((order) => {
    ...
    const dietType = order.ordername || 
      (order.diet.length > 0 
        ? `${order.diet[0].mealType.charAt(0).toUpperCase() + order.diet[0].mealType.slice(1)} Plan` 
        : "Custom Diet Plan")
    ...
    return {
      ...
      deliveryDate: order.startdate ? new Date(order.startdate).toISOString().split('T')[0] : "N/A",
...
export function useDietData() {
  const [selectedRow, setSelectedRow] = useState<{
      ...
      fat: number
      deliveryDate: string
      transactionId: string; id: string; diet: string; status: string 
} | null>(null)
  ...
  const [monthlyTrendData, setMonthlyTrendData] = useState<{ month: string; orders: number }[]>([])
  const [isLoading, setIsLoading] = useState(true)

  // Handle row selection
  const handleRowSelect = (row: { id: string; diet: string; status: string }) => {
